    ...
    @if (Route::has('login'))
        <div class="flex items-center sm:fixed sm:top-0 sm:right-0 px-8 py-2 text-right text-sm">
            @auth
                ...
                <a href="{{ url('/dashboard') }}" class="flex items-center text-white hover:text-amber-400 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">
                    Dashboard
                    <span class="material-icons-outlined text-sm ml-1">account_circle</span>
                </a>
            @else
                {{-- Log in --}}
                <a href="{{ route('login') }}" class="flex items-center text-white hover:text-amber-400 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">
                    Log in
                    <span class="material-icons-outlined text-sm ml-1">login</span>
                </a>
                @if (Route::has('register'))
                    {{-- Register --}}
                    <a href="{{ route('register') }}" class="flex items-center ml-4 text-white hover:text-amber-400 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">
                        Register
